...
if (isset($_POST['issent'])) {
	$errmsg = "Les champs suivants doivent être obligatoirement remplis :<br/><br/>";
	$message = $errmsg;
	$controle = false;
	...
	else {$errmsg.= "Madame // Monsieur<br />";  $_SESSION['sexe']=false;}
	if(empty($_POST['nom'])) 
		{$errmsg.= "votre nom<br />"; $_SESSION['nom'] = NULL; $_SESSION['ctrlNom']=false;}
	else {$_SESSION['nom'] = htmlspecialchars($_POST['nom']); $_SESSION['ctrlNom']=true;}
	if(empty($_POST['prenom'])) 
		{$errmsg.= "votre prénom<br />"; $_SESSION['prenom'] = NULL; $_SESSION['ctrlPrenom']=false;}
	else {$_SESSION['prenom'] = htmlspecialchars($_POST['prenom']); $_SESSION['ctrlPrenom']=true;}
	if (isset($_POST['email']))
	{
	 if(empty($_POST['email'])) 
		{$errmsg.= 'votre adresse e-mail<br />'; $_SESSION['email'] = NULL; $_SESSION['ctrlEmail']=false;}
	 else {
	    if (!preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $_POST['email']))
			{$errmsg.= 'Le format de l\'adresse e-mail n\'est pas valide !<br />';  
			$_SESSION['email'] = NULL; $_SESSION['ctrlEmail']=false;}
		else{$_SESSION['email'] = htmlspecialchars($_POST['email']); $_SESSION['ctrlEmail']=true;}
	 }
	}
	if(empty($_POST['adresse'])) 
		{$errmsg.= "votre adresse<br />"; $_SESSION['adresse'] = NULL; $_SESSION['ctrlAdresse']=false;}
	else {$_SESSION['adresse'] = htmlspecialchars($_POST['adresse']); $_SESSION['ctrlAdresse']=true;}
	if(empty($_POST['codepostal'])) 
		{$errmsg.= "votre code postal<br />"; $_SESSION['codepostal'] = NULL; $_SESSION['ctrlCodePostal']=false;}
	else {$_SESSION['codepostal'] = htmlspecialchars($_POST['codepostal']); $_SESSION['ctrlCodePostal']=true;}
	if(empty($_POST['ville'])) 
		{$errmsg.= "votre ville<br />"; $_SESSION['ville'] = NULL; $_SESSION['ctrlVille']=false;}
	else {$_SESSION['ville'] = htmlspecialchars($_POST['ville']); $_SESSION['ctrlVille']=true;}
	if(empty($_POST['pays'])) 
		{$errmsg.= "votre pays<br />"; $_SESSION['pays'] = NULL; $_SESSION['ctrlPays']=false;}
   ...
   if ($controle==true) {
    ...
    echo "<h3>Le formulaire a bien été envoyé</h3>";
	echo "Nous vous répondrons dans les meilleurs délais à l'adresse ".$_SESSION['email']."<br />";
	echo "</div>"; 
	}
}